...
void sgtl5000_i2c_write_register(h_sgtl5000_t * h_sgtl5000, uint16_t reg, uint16_t value) {
    uint8_t data[2];
    data[0] = (value >> 8) & 0xFF;  // Partie haute
    data[1] = value & 0xFF;         // Partie basse

    // Écriture dans le registre via I2C
    HAL_I2C_Mem_Write(h_sgtl5000->hi2c, SGTL5000_I2C_ADDR,reg, I2C_MEMADD_SIZE_16BIT, data, 2, HAL_MAX_DELAY);
}

void sgtl5000_i2c_set_bit(h_sgtl5000_t * h_sgtl5000, uint16_t reg, uint16_t mask) {
    uint16_t value;

    // Lire la valeur actuelle
    sgtl5000_i2c_read_register(h_sgtl5000, reg, &value);  // Assurez-vous d'avoir cette fonction
    value |= mask;  // Définir les bits spécifiés
    sgtl5000_i2c_write_register(h_sgtl5000, reg, value);  // Écrire la nouvelle valeur
}

void sgtl5000_i2c_read_register(h_sgtl5000_t * h_sgtl5000, uint16_t reg, uint16_t *value) {
    uint8_t data[2];

    // Lecture du registre via I2C
    HAL_I2C_Mem_Read(h_sgtl5000->hi2c, SGTL5000_I2C_ADDR,
                     reg, I2C_MEMADD_SIZE_16BIT, data, 2, HAL_MAX_DELAY);
    *value = (data[0] << 8) | data[1];  // Combiner les deux octets
}

void sgtl5000_i2c_clear_bit(h_sgtl5000_t *h_sgtl5000, uint16_t reg, uint16_t mask) {
    uint16_t value;

    // Lire la valeur actuelle du registre
    sgtl5000_i2c_read_register(h_sgtl5000, reg, &value);

    // Effacer les bits spécifiés par le masque
    value &= ~mask;

    // Écrire la nouvelle valeur dans le registre
    sgtl5000_i2c_write_register(h_sgtl5000, reg, value);
}
