...
{
    for (;;) {
        // Attendre que l'interruption libère le semaphore
        if (xSemaphoreTake(xSemaphore_UART, portMAX_DELAY) == pdTRUE) {
            // Exécuter le shell une fois qu'un événement de réception est signalé
            shell_run(&h_shell);  // Ou tout autre code pour exécuter le shell
        }
    }
}
*/
